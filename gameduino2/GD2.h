...
#define PROGMEM
#define pgm_read_byte_near(x) (*((uint8_t *)((void *)(x))))
...
public:
  ...
  void begin(uint8_t options = (GD_CALIBRATE | GD_TRIM));
  ...
  void cmd_stop(void);
  void cmd_swap(void);
  void cmd_text(int16_t x, int16_t y, byte font, uint16_t options, const char *s, int inc = 30);
  ...
  uint32_t loadMem(const char *filename, uint32_t address);
