...
{
  GD.loadptr = (GD.loadptr + 1) & ~1;
  GD.cmd_loadimage(GD.loadptr, OPT_NODL);   //ML change.. was OPT_NODL
  GD.load(filename);
  uint32_t ptr, w, h;
...
void Bitmap::defaults(uint8_t f)
{
  ...
  center.x = size.x / 2;
  center.y = size.y / 2;
  GD.loadptr += (unsigned long)((size.x << 1) >> pgm_read_byte_near(bpltab + f)) * size.y;
}
...
/*#if defined(ARDUINO) && !defined(__DUE__)
    if ((EEPROM.read(0) != 0x7c)) {		//ML change to force CAL.. should be 0x7c
      self_calibrate();
      for (int i = 0; i < 24; i++)
    ...
    //static const byte canned_calibration[24] = {
    //    0xB4, 0xFF, 0xFF, 0xFF, 0x72, 0xA6, 0xFF, 0xFF,
    //    0xBB, 0x6E, 0x0A, 0x01, 0xDF, 0x5F, 0x00, 0x00,
    //    0xE6, 0xFF, 0xFF, 0xFF, 0x02, 0x36, 0x02, 0x00 };
    

    //static const byte canned_calibration[24] = {
    //  0x2E, 0x0A, 0x00, 0x00, 0x52, 0xFE, 0xFE, 0xFF,
    //  0x78, 0xBB, 0x08, 0x01, 0x73, 0xFD, 0x00, 0x00,
    //  0xA6, 0xFD, 0xFF, 0xFF, 0x90, 0xA5, 0x04, 0x00 };
	
	//static const byte canned_calibration[24] = {  
	//0xc0,	0xf4,	0x00,	0x00,	0xdb,	0x07,
	//0x00,	0x00,	0xc3,	0xed,	0xf1,	0xff,	
	//0x8b,	0x01,	0x00,	0x0,	0x6c,	0x06,	
	//0x01,	0x00,	0xda,	0x38,	0xf1,	0xff };      

static const byte canned_calibration[24] = {    //new values for Riverdi
	0x55,	0x15,	0x01,	0x00,	0x71,	0x00,
	0x00,	0x00,	0x8f,	0x93,	0xef,	0xff,	
	0x00,	0x00,	0x00,	0x00,	0x00,	0x00,	
	0x01,	0x00,	0x00,	0x00,	0xfc,	0xff };      


    //dpm 3_16_2020 added production Riverdi cal    
    for (int i = 0; i < 24; i++)
...
//  byte config[24];
//  for (int i = 0; i < 24; i++)
//    config[i] = GDTR.rd(REG_TOUCH_TRANSFORM_A + i);
//  (void)config;
}
//////////////////////////////////////////////////////////////////////////////////////////////////
// void GDClass::bitmapML(const char* filename, uint8_t handle) {      //////////wrapper to use Bitmap class
  // Bitmap BM;
  // BM.fromfile(filename, 0);
  // BM.bind(handle);
// }
///////////////////////////////////////////////////////////////////////////////////////////////////
// void GDClass::load6bitmapsML(const char* filename1, uint8_t handle1, const char* filename2, uint8_t handle2, const char* filename3, uint8_t handle3, const char* filename4, uint8_t handle4, const char* filename5, uint8_t handle5, const char* filename6, uint8_t handle6) {
  // Bitmap ML1, ML2, ML3, ML4;// ML5, ML6;
  // ML1.fromfile(filename1, 0);
  // ML1.bind(handle1);
  // ML2.fromfile(filename2, 0);
  // ML2.bind(handle2);
  // ML3.fromfile(filename3, 0);
  // ML3.bind(handle3);
   // ML4.fromfile(filename4, 0);
   // ML4.bind(handle4);
  // ML5.fromfile(filename5, 0);
  // ML5.bind(handle5);
  // ML6.fromfile(filename6, 0);
  // ML6.bind(handle6);
// }
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void GDClass::cmd_text(int16_t x, int16_t y, byte font, uint16_t options, const char *s, int inc) {
  char buf[128];
  int idx = 0;
...
{
  char full_name[128] = DRV_SD;
  strcat(full_name, filename);

  FIL f;
  FRESULT result = f_open(&f, full_name, FA_READ);
  if (result != FR_OK)
    alert(full_name);
  ...
  UINT tot = 0;
  UINT n;
  while ((result = f_read(&f, buf, 512, &n)) == FR_OK) {
  ...
  (void)tot;*/
	...
	while (bytes_read < size) {
		result = f_read(&f, buf, bytes_to_read, &br);
		if (result != FR_OK)
			break;
		bytes_read += br;
		GDTR.cmd_n((byte *)buf, (br + 3) & ~3);
		if (size - bytes_read < 512)
...
uint32_t GDClass::loadMem(const char *filename, uint32_t address)
{
  char full_name[128] = DRV_SD;
  strcat(full_name, filename);
  ...
  if (result != FR_OK)
	...
	while (bytes_read < size) {
		...
		if (result != FR_OK)
			break;
		GD.wr_n(address + bytes_read, (byte *)buf, br);
		bytes_read += br;
		if (size - bytes_read < 512)
			bytes_to_read = size - bytes_read;
	}
...
{
  {
    ...
    for (int ly = 0; ly < GD.h; ly++) {
      ...
      for (int x = 0; x < GD.w; x += 8) {
        ...
        for (int i = 0; i < 8; i++) {
          block[i].b = spi_transfer_byte(0xff);
          block[i].g = spi_transfer_byte(0xff);
          block[i].r = spi_transfer_byte(0xff);
          block[i].a = spi_transfer_byte(0xff);
        }
